...
if ($method === 'POST') {
    ...
    // Rate limit: máx 10 reseñas por IP cada 10 minutos
    checkRateLimit('review_post', 10, 600);
    ...
    if ($result['success']) {
        auditLog('review_upsert', 'review', null, ['business_id' => $bId]);
...
if ($method === 'DELETE') {
    ...
    if ($reviewId > 0) {
        ...
        if ($result['success']) {
            auditLog('review_delete', 'review', $reviewId);
        }
